...
func AddTags(c *gin.Context) {
	...
	valid.Required(createBy, "created_by").Message("创建人的名字不能为空")
	valid.MaxSize(createBy, 100, "created_by").Message("创建人的名字最长100字符")
...
func EditTag(c *gin.Context) {
	...
	if arg := c.Query("state"); arg != "" {
		state = com.StrTo(arg).MustInt()
		valid.Range(state, 0, 1, "state").Message("状态只能是0 或者 1")
	}

	valid.Required(id, "id").Message("ID不能为空")
	valid.Required(modifiedBy, "modified_by").Message("修改人不能为空")
	valid.MaxSize(modifiedBy, 100, "modified_by").Message("修改人最长字符不超过100")
